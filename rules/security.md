...
- **Never trust `present?` to mean "has a real value"** — because `"   ".present?` and `["", nil].present?` are both true; strip/coerce input at the boundary before guarding on it. *(Extend per host.)*
