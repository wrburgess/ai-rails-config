...
- **Never act on a remote-authoritative namespace from stale local state** — because ADR numbers, issue numbers, and `main`'s commit graph live on the remote, so a value computed from a possibly-behind local checkout collides the instant a parallel branch moved first (a gap, a duplicate, a stale rebase base): sync-before-create, search-before-file, fetch-before-rebase. *(Provenance: #133 / #131; extend per host.)*
