...
**Applies to:** `bin/`, `scripts/` (bundled and CLI scripts shipped in the Config Bundle or a Host App)
...
- **Dependency-free by default.** A bundled script must run on a bare interpreter in an unknown CI — use the standard library only (no gems, no bundler) unless the host explicitly opts in.
